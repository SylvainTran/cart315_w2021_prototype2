...
Why did I add the command line interface? The main reason is that of a prototyping need. There were ideas from the first playtesting session that I hadn't had the time to test properly, and implementing them at every stage was impossible with the time I had. The second reason is that I felt like I needed to try that idea with my game.
